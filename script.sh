...
aws s3api create-bucket --bucket soap-transformer-$ACC_ID
...
aws cloudformation create-stack --stack-name soap-pattern-stack --template-body file://cloudformation-template.json --capabilities CAPABILITY_IAM --parameters ParameterKey=APIEndpoint,ParameterValue=$endpoint
cd ..
# sam build
